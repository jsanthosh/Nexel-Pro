...
QVariant FormulaEngine::evaluateFunction(const QString& fn, const std::vector<QVariant>& args) {
    ...
    // Batch 2
    if (fn == "SINH") return funcSINH(args);
    if (fn == "COSH") return funcCOSH(args);
    if (fn == "TANH") return funcTANH(args);
    if (fn == "FACTDOUBLE") return funcFACTDOUBLE(args);
    if (fn == "MULTINOMIAL") return funcMULTINOMIAL(args);
    if (fn == "BASE") return funcBASE(args);
    if (fn == "DECIMAL") return funcDECIMAL(args);
    if (fn == "ROMAN") return funcROMAN(args);
    if (fn == "ARABIC") return funcARABIC(args);
    if (fn == "SUBTOTAL") return funcSUBTOTAL(args);
    if (fn == "COMBINA") return funcCOMBINA(args);
    if (fn == "SERIESSUM") return funcSERIESSUM(args);
    if (fn == "XMATCH") return funcXMATCH(args);
    if (fn == "SORTBY") return funcSORTBY(args);
    if (fn == "VSTACK") return funcVSTACK(args);
    if (fn == "HSTACK") return funcHSTACK(args);
    if (fn == "TAKE") return funcTAKE(args);
    if (fn == "DROP") return funcDROP(args);
    if (fn == "DOLLAR") return funcDOLLAR(args);
    if (fn == "LENB") return funcLENB(args);
    if (fn == "FINDB") return funcFINDB(args);
    if (fn == "SEARCHB") return funcSEARCHB(args);
    if (fn == "VALUETOTEXT") return funcVALUETOTEXT(args);
    if (fn == "STDEV.P" || fn == "STDEVP") return funcSTDEVP(args);
    if (fn == "STDEV.S") return funcSTDEV(args);
    if (fn == "VAR.P" || fn == "VARP") return funcVARP(args);
    if (fn == "VAR.S") return funcVAR(args);
    if (fn == "PERCENTILE.INC") return funcPERCENTILE_INC(args);
    if (fn == "PERCENTILE.EXC") return funcPERCENTILE_EXC(args);
    if (fn == "QUARTILE.INC") return funcQUARTILE_INC(args);
    if (fn == "RANK.EQ") return funcRANK_EQ(args);
    if (fn == "RANK.AVG") return funcRANK_AVG(args);
    if (fn == "GEOMEAN") return funcGEOMEAN(args);
    if (fn == "HARMEAN") return funcHARMEAN(args);
    if (fn == "TRIMMEAN") return funcTRIMMEAN(args);
    if (fn == "DEVSQ") return funcDEVSQ(args);
    if (fn == "AVEDEV") return funcAVEDEV(args);
    if (fn == "RSQ") return funcRSQ(args);
    if (fn == "FREQUENCY") return funcFREQUENCY(args);
    if (fn == "RATE") return funcRATE(args);
    if (fn == "XNPV") return funcXNPV(args);
    if (fn == "XIRR") return funcXIRR(args);
    if (fn == "CUMIPMT") return funcCUMIPMT(args);
    if (fn == "CUMPRINC") return funcCUMPRINC(args);
    if (fn == "PDURATION") return funcPDURATION(args);
    if (fn == "RRI") return funcRRI(args);
    if (fn == "FVSCHEDULE") return funcFVSCHEDULE(args);
    if (fn == "DAYS360") return funcDAYS360(args);
    if (fn == "NETWORKDAYS.INTL") return funcNETWORKDAYS_INTL(args);
    if (fn == "WORKDAY.INTL") return funcWORKDAY_INTL(args);
    if (fn == "ISFORMULA") return funcISFORMULA(args);
    if (fn == "ISREF") return funcISREF(args);
    // Batch 3
    if (fn == "TEXTSPLIT") return funcTEXTSPLIT(args);
    if (fn == "WRAPROWS") return funcWRAPROWS(args);
    if (fn == "WRAPCOLS") return funcWRAPCOLS(args);
    if (fn == "TOROW") return funcTOROW(args);
    if (fn == "TOCOL") return funcTOCOL(args);
    if (fn == "CHOOSECOLS") return funcCHOOSECOLS(args);
    if (fn == "CHOOSEROWS") return funcCHOOSEROWS(args);
    if (fn == "EXPAND") return funcEXPAND(args);
    if (fn == "RANDARRAY") return funcRANDARRAY(args);
    if (fn == "NORM.DIST") return funcNORM_DIST(args);
    if (fn == "NORM.INV") return funcNORM_INV(args);
    if (fn == "NORM.S.DIST") return funcNORM_S_DIST(args);
    if (fn == "NORM.S.INV") return funcNORM_S_INV(args);
    if (fn == "BINOM.DIST") return funcBINOM_DIST(args);
    if (fn == "POISSON.DIST") return funcPOISSON_DIST(args);
    if (fn == "CONFIDENCE.NORM") return funcCONFIDENCE_NORM(args);
    if (fn == "MODE.SNGL") return funcMODE_SNGL(args);
    if (fn == "MIRR") return funcMIRR(args);
    if (fn == "DB") return funcDB(args);
    if (fn == "DDB") return funcDDB(args);
    if (fn == "SYD") return funcSYD(args);
    if (fn == "DOLLARDE") return funcDOLLARDE(args);
    if (fn == "DOLLARFR") return funcDOLLARFR(args);
    if (fn == "BIN2DEC") return funcBIN2DEC(args);
    if (fn == "DEC2BIN") return funcDEC2BIN(args);
    if (fn == "HEX2DEC") return funcHEX2DEC(args);
    if (fn == "DEC2HEX") return funcDEC2HEX(args);
    if (fn == "OCT2DEC") return funcOCT2DEC(args);
    if (fn == "DEC2OCT") return funcDEC2OCT(args);
    if (fn == "CONVERT") return funcCONVERT(args);
    if (fn == "DELTA") return funcDELTA(args);
    if (fn == "GESTEP") return funcGESTEP(args);
    if (fn == "DSUM") return funcDSUM(args);
    if (fn == "DAVERAGE") return funcDAVERAGE(args);
    if (fn == "DCOUNT") return funcDCOUNT(args);
    if (fn == "DCOUNTA") return funcDCOUNTA(args);
    if (fn == "DMIN") return funcDMIN(args);
    if (fn == "DMAX") return funcDMAX(args);
    if (fn == "DGET") return funcDGET(args);
    if (fn == "DPRODUCT") return funcDPRODUCT(args);
    if (fn == "DSTDEV") return funcDSTDEV(args);
    if (fn == "DVAR") return funcDVAR(args);
    if (fn == "ENCODEURL") return funcENCODEURL(args);
    // Batch 4 — Sprint 1 completion
    if (fn == "LET") return funcLET(args);
    if (fn == "LAMBDA") return funcLAMBDA(args);
    if (fn == "MAP") return funcMAP(args);
    if (fn == "REDUCE") return funcREDUCE(args);
    if (fn == "SCAN") return funcSCAN(args);
    if (fn == "MAKEARRAY") return funcMAKEARRAY(args);
    if (fn == "BYROW") return funcBYROW(args);
    if (fn == "BYCOL") return funcBYCOL(args);
    if (fn == "ISOMITTED") return funcISOMITTED(args);
    if (fn == "OFFSET") return funcOFFSET(args);
    if (fn == "INDIRECT") return funcINDIRECT(args);
    if (fn == "LOOKUP") return funcLOOKUP(args);
    if (fn == "AREAS") return funcAREAS(args);
    if (fn == "T.DIST") return funcT_DIST(args);
    if (fn == "T.INV") return funcT_INV(args);
    if (fn == "T.DIST.2T") return funcT_DIST_2T(args);
    if (fn == "T.INV.2T") return funcT_INV_2T(args);
    if (fn == "CHISQ.DIST") return funcCHISQ_DIST(args);
    if (fn == "F.DIST") return funcF_DIST(args);
    if (fn == "EXPON.DIST") return funcEXPON_DIST(args);
    if (fn == "GAMMA.DIST") return funcGAMMA_DIST(args);
    if (fn == "WEIBULL.DIST") return funcWEIBULL_DIST(args);
    if (fn == "SKEW") return funcSKEW(args);
    if (fn == "KURT") return funcKURT(args);
    if (fn == "PROB") return funcPROB(args);
    if (fn == "CONFIDENCE.T") return funcCONFIDENCE_T(args);
    if (fn == "BIN2HEX") return funcBIN2HEX(args);
    if (fn == "BIN2OCT") return funcBIN2OCT(args);
    if (fn == "HEX2BIN") return funcHEX2BIN(args);
    if (fn == "HEX2OCT") return funcHEX2OCT(args);
    if (fn == "OCT2BIN") return funcOCT2BIN(args);
    if (fn == "OCT2HEX") return funcOCT2HEX(args);
    if (fn == "BITAND") return funcBITAND(args);
    if (fn == "BITOR") return funcBITOR(args);
    if (fn == "BITXOR") return funcBITXOR(args);
    if (fn == "BITLSHIFT") return funcBITLSHIFT(args);
    if (fn == "BITRSHIFT") return funcBITRSHIFT(args);
    if (fn == "ARRAYTOTEXT") return funcARRAYTOTEXT(args);
    if (fn == "FORMULATEXT") return funcFORMULATEXT(args);
    if (fn == "SHEET") return funcSHEET(args);
    if (fn == "SHEETS") return funcSHEETS(args);
    if (fn == "AGGREGATE") return funcAGGREGATE(args);
    if (fn == "MMULT") return funcMMULT(args);
    if (fn == "MUNIT") return funcMUNIT(args);
    // Batch 5 — Sprint 1 100%
    if (fn == "ASINH") return funcASINH(args);
    if (fn == "ACOSH") return funcACOSH(args);
    if (fn == "ATANH") return funcATANH(args);
    if (fn == "PERMUTATIONA") return funcPERMUTATIONA(args);
    if (fn == "SUMX2MY2") return funcSUMX2MY2(args);
    if (fn == "SUMX2PY2") return funcSUMX2PY2(args);
    if (fn == "SUMXMY2") return funcSUMXMY2(args);
    if (fn == "COVARIANCE.S") return funcCOVARIANCE_S(args);
    if (fn == "COVARIANCE.P") return funcCOVARIANCE_P(args);
    if (fn == "STEYX") return funcSTEYX(args);
    if (fn == "STANDARDIZE") return funcSTANDARDIZE(args);
    if (fn == "FISHER") return funcFISHER(args);
    if (fn == "FISHERINV") return funcFISHERINV(args);
    if (fn == "PERCENTRANK.INC") return funcPERCENTRANK_INC(args);
    if (fn == "PERCENTRANK.EXC") return funcPERCENTRANK_EXC(args);
    if (fn == "QUARTILE.EXC") return funcQUARTILE_EXC(args);
    if (fn == "VARA") return funcVARA(args);
    if (fn == "STDEVA") return funcSTDEVA(args);
    if (fn == "VARPA") return funcVARPA(args);
    if (fn == "STDEVPA") return funcSTDEVPA(args);
    if (fn == "Z.TEST") return funcZ_TEST(args);
    if (fn == "T.TEST") return funcT_TEST(args);
    if (fn == "F.TEST") return funcF_TEST(args);
    if (fn == "CHISQ.TEST") return funcCHISQ_TEST(args);
    if (fn == "ERF" || fn == "ERF.PRECISE") return funcERF(args);
    if (fn == "ERFC" || fn == "ERFC.PRECISE") return funcERFC(args);
    if (fn == "COMPLEX") return funcCOMPLEX(args);
    if (fn == "IMAGINARY") return funcIMAGINARY(args);
    if (fn == "IMREAL") return funcIMREAL(args);
    if (fn == "BETA.DIST") return funcBETA_DIST(args);
    if (fn == "BETA.INV") return funcBETA_INV(args);
    if (fn == "GAMMA.INV") return funcGAMMA_INV(args);
    if (fn == "CHISQ.INV") return funcCHISQ_INV(args);
    if (fn == "F.INV") return funcF_INV(args);
    if (fn == "BINOM.INV") return funcBINOM_INV(args);
    if (fn == "LOGNORM.DIST") return funcLOGNORM_DIST(args);
    if (fn == "LOGNORM.INV") return funcLOGNORM_INV(args);
    if (fn == "NEGBINOM.DIST") return funcNEGBINOM_DIST(args);
    if (fn == "HYPGEOM.DIST") return funcHYPGEOM_DIST(args);
    // Legacy compatibility aliases
    if (fn == "CONCAT") return funcCONCAT(args);
    if (fn == "REPLACE") return funcREPLACE(args);
    if (fn == "NORMINV") return funcNORM_INV(args);
    if (fn == "NORMDIST") return funcNORM_DIST(args);
    if (fn == "TDIST") return funcT_DIST(args);
    if (fn == "TINV") return funcT_INV(args);
    if (fn == "FDIST") return funcF_DIST(args);
    if (fn == "FINV") return funcF_INV(args);
    if (fn == "BETADIST") return funcBETA_DIST(args);
    if (fn == "BETAINV") return funcBETA_INV(args);
    if (fn == "CHIINV") return funcCHISQ_INV(args);
