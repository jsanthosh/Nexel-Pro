...
inline const QMap<QString, FormulaFuncInfo>& formulaRegistry() {
    static const QMap<QString, FormulaFuncInfo> reg = {
        ...
        {"IMABS", {"IMABS", "Returns the absolute value (modulus) of a complex number",
            "IMABS(inumber)",
            {{"inumber", false, "A complex number in the form \"a+bi\""}}}},
        ...
        {"IMCONJUGATE", {"IMCONJUGATE", "Returns the complex conjugate of a complex number",
            "IMCONJUGATE(inumber)",
            {{"inumber", false, "A complex number in the form \"a+bi\""}}}},
        {"IMSUM", {"IMSUM", "Returns the sum of two or more complex numbers",
            "IMSUM(inumber1, [inumber2], ...)",
            {{"inumber1", false, "The first complex number"},
             {"inumber2", true, "Additional complex numbers to add"}}}},
        {"IMSUB", {"IMSUB", "Returns the difference of two complex numbers",
            "IMSUB(inumber1, inumber2)",
            {{"inumber1", false, "The complex number to subtract from"},
             {"inumber2", false, "The complex number to subtract"}}}},
        {"IMPRODUCT", {"IMPRODUCT", "Returns the product of two or more complex numbers",
            "IMPRODUCT(inumber1, [inumber2], ...)",
            {{"inumber1", false, "The first complex number"},
             {"inumber2", true, "Additional complex numbers to multiply"}}}},
        {"IMDIV", {"IMDIV", "Returns the quotient of two complex numbers",
            "IMDIV(inumber1, inumber2)",
            {{"inumber1", false, "The complex numerator"},
             {"inumber2", false, "The complex denominator"}}}},
        {"IMPOWER", {"IMPOWER", "Returns a complex number raised to a power",
            "IMPOWER(inumber, number)",
            {{"inumber", false, "A complex number"},
             {"number", false, "The power to raise the complex number to"}}}},
        ...
        {"MINVERSE", {"MINVERSE", "Returns the inverse of a square matrix",
            "MINVERSE(array)",
            {{"array", false, "A square numeric array or range"}}}},
        {"MDETERM", {"MDETERM", "Returns the determinant of a square matrix",
            "MDETERM(array)",
            {{"array", false, "A square numeric array or range"}}}},

        // === Bessel Functions ===
        {"BESSELI", {"BESSELI", "Returns the modified Bessel function of the first kind I_n(x)",
            "BESSELI(x, n)",
            {{"x", false, "The value at which to evaluate"},
             {"n", false, "The order of the Bessel function (non-negative integer)"}}}},
        {"BESSELJ", {"BESSELJ", "Returns the Bessel function of the first kind J_n(x)",
            "BESSELJ(x, n)",
            {{"x", false, "The value at which to evaluate"},
             {"n", false, "The order of the Bessel function (non-negative integer)"}}}},
        {"BESSELK", {"BESSELK", "Returns the modified Bessel function of the second kind K_n(x)",
            "BESSELK(x, n)",
            {{"x", false, "The value at which to evaluate (must be positive)"},
             {"n", false, "The order of the Bessel function (non-negative integer)"}}}},
        {"BESSELY", {"BESSELY", "Returns the Bessel function of the second kind Y_n(x)",
            "BESSELY(x, n)",
            {{"x", false, "The value at which to evaluate (must be positive)"},
             {"n", false, "The order of the Bessel function (non-negative integer)"}}}},

        // === Statistical ===
        {"PHI", {"PHI", "Returns the value of the standard normal density function",
            "PHI(x)",
            {{"x", false, "The number for which to calculate the density"}}}},
        {"GAUSS", {"GAUSS", "Returns the probability between the mean and z standard deviations",
            "GAUSS(z)",
            {{"z", false, "A number of standard deviations from the mean"}}}},
        {"MODE.MULT", {"MODE.MULT", "Returns an array of the most frequently occurring values",
            "MODE.MULT(number1, [number2], ...)",
            {{"number1", false, "The first number, cell reference, or range"},
             {"number2", true, "Additional numbers, references, or ranges"}}}},

        // === Financial ===
        {"VDB", {"VDB", "Returns variable declining balance depreciation for a specific period",
            "VDB(cost, salvage, life, start_period, end_period, [factor], [no_switch])",
            {{"cost", false, "The initial cost of the asset"},
             {"salvage", false, "The value at the end of the depreciation"},
             {"life", false, "The number of periods over which the asset is depreciated"},
             {"start_period", false, "The starting period for depreciation calculation"},
             {"end_period", false, "The ending period for depreciation calculation"},
             {"factor", true, "The rate of balance decline. Default is 2 (double-declining)"},
             {"no_switch", true, "If TRUE, does not switch to straight-line method"}}}},

        // === Information ===
        {"CELL", {"CELL", "Returns information about the formatting, location, or contents of a cell",
            "CELL(info_type, [reference])",
            {{"info_type", false, "A text value specifying the type of info (type, row, col, address, contents, etc.)"},
             {"reference", true, "The cell to get information about"}}}},
        {"INFO", {"INFO", "Returns information about the current operating environment",
            "INFO(type_text)",
            {{"type_text", false, "Text specifying the type of info (osversion, system, directory, numfile, origin, recalc, release)"}}}},

        // === Math Trig ===
        {"ACOT", {"ACOT", "Returns the arccotangent of a number",
            "ACOT(number)",
            {{"number", false, "The cotangent of the angle you want"}}}},
        {"ACOTH", {"ACOTH", "Returns the inverse hyperbolic cotangent of a number",
            "ACOTH(number)",
            {{"number", false, "The value whose inverse hyperbolic cotangent you want. Must have absolute value > 1"}}}},
        {"COT", {"COT", "Returns the cotangent of an angle specified in radians",
            "COT(number)",
            {{"number", false, "The angle in radians"}}}},
        {"COTH", {"COTH", "Returns the hyperbolic cotangent of a number",
            "COTH(number)",
            {{"number", false, "The value in radians"}}}},
        {"CSC", {"CSC", "Returns the cosecant of an angle specified in radians",
            "CSC(number)",
            {{"number", false, "The angle in radians"}}}},
        {"CSCH", {"CSCH", "Returns the hyperbolic cosecant of a number",
            "CSCH(number)",
            {{"number", false, "The value in radians"}}}},
        {"SEC", {"SEC", "Returns the secant of an angle specified in radians",
            "SEC(number)",
            {{"number", false, "The angle in radians"}}}},
        ...
        // === Database ===
        {"DSTDEVP", {"DSTDEVP", "Calculates the standard deviation of a population based on the entire population of entries matching criteria",
            "DSTDEVP(database, field, criteria)",
            {{"database", false, "The range of cells that makes up the database"},
             {"field", false, "The column to use in the function"},
             {"criteria", false, "The range containing conditions to match"}}}},
        {"DVARP", {"DVARP", "Calculates variance of a population based on the entire population of entries matching criteria",
