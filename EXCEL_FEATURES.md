...
>
> **Last audit**: 2026-03-30 — comprehensive codebase audit against native Qt/C++ app
...
- ✅ Must handle 1M+ rows without performance degradation (handles 20M+ via VirtualScrollBar)
...
### 1.3 Zoom [P1]
- ✅ Range: 10% to 400% (zoomIn/zoomOut/resetZoom)
...
- ✅ Visual divider line for frozen area (theme-colored freeze lines)

### 1.5 Grid Lines [P1]
- ✅ Show/hide gridlines per sheet (setGridlinesVisible)
- ⬜ Print gridlines toggle (separate from display) — no print support yet
- ✅ Gridline color customization (via Theme)
...
- 🟡 Name box for direct cell jump (GoToDialog exists, not integrated into toolbar)
- ✅ Name box shows current cell address (via FormulaBar)
...
- ✅ Ctrl + click to add disjoint cell to selection (ExtendedSelection mode)
...
- ✅ Ctrl+G / F5: Go To dialog (GoToDialog)
- ⬜ Go To named ranges
...
- ✅ Ctrl+Arrow → jump to edge of data region (O(chunks) via ColumnStore)
...
- 🟡 Name box on left showing cell address or named range (shows address, no named range)
...
- ✅ Function name autocomplete with dropdown (FormulaPopupDelegate with [fx] badge)
- ✅ Function signature tooltip (parameters with types, rich HTML formatting)
- ⬜ Named range autocomplete in formulas
- ⬜ Table column reference autocomplete (Table1[...)

### 3.4 Formula Syntax Highlighting [P1]
- ✅ Color-coded cell references (each referenced range gets unique color, 8-color cycle)
...
### 3.6 AutoFill [P1]
- ✅ Drag fill handle to extend series (performFillSeries)
...
- ✅ **Validation types**:
  ...
  - ✅ List (dropdown from comma-separated or range reference, with sort options + custom colors)
  - ✅ Date (same operators)
  - ✅ Time (same operators)
...
### 4.2 Resize [P1]
- ✅ Drag row/column border to resize (Interactive resize mode)
- ✅ Double-click border to auto-fit content (autofitRow/autofitColumn)
...
### 4.3 Hide / Unhide [P1]
- ✅ Hide selected rows/columns (setRowHidden/setColumnHidden)
...
- ✅ **Number**: decimal places (0-10), thousands separator, negative number display (red, parentheses, minus, red with parentheses)
- ✅ **Currency**: symbol selection (50+ symbols), symbol position, decimal places, negative display
- ✅ **Accounting**: aligned currency symbols, aligned decimal points, dashes for zero
- ✅ **Date**: 10+ formats (MM/DD/YYYY, DD/MM/YYYY, YYYY-MM-DD, MMM D YYYY, etc.)
- ✅ **Time**: h:mm, h:mm:ss, h:mm AM/PM, h:mm:ss AM/PM, elapsed time [h]:mm:ss
...
- ✅ **Special**: zip code, zip+4, phone, SSN
...
- ✅ **Scaling**: `,` at end divides by 1000 per comma
...
- ✅ **Date/time codes**: `yyyy`, `mm`, `dd`, `hh`, `mm`, `ss`, `AM/PM`, `ddd`, `dddd`, `mmm`, `mmmm`
...
- ✅ Font family (dropdown with preview via QFontComboBox)
- ✅ Font size (6pt to 72pt via spinbox)
- ✅ Bold (Ctrl+B)
- ✅ Italic (Ctrl+I)
- ✅ Underline: single (Ctrl+U)
- ✅ Strikethrough
- ✅ Font color (theme colors 6x10 grid + standard 10 + grayscale 10 + custom RGB)
...
- ✅ **Horizontal**: Left, Center, Right; ⬜ Fill, Justify, Center Across Selection, Distributed
- ✅ **Vertical**: Top, Middle, Bottom; ⬜ Justify, Distributed
- ✅ **Text rotation**: angle from -90° to +90°, plus vertical stacked text (270°)
- ✅ **Indent**: increase/decrease levels (12px per level)
- ✅ **Wrap text**: auto-wrap with row height auto-adjust
- ✅ **Shrink to fit**: auto-reduce font size to fit cell width
...
- ✅ **Preset border buttons**: box/outline, all borders/inside, no border
...
- ✅ Single-click: copy format, paste once (m_formatPainterActive)
...
### 10.1 Undo [P1]
- ✅ Multiple levels of undo (memory-capped history)
...
- ✅ Cycle detection with user warning (DFS-based)
...
- ✅ Skip unchanged branches (topological ordering via Kahn's algorithm)
- ✅ Dynamic calculation chain ordering (getRecalcOrder / getRecalcLevels)
- ✅ Volatile function handling: always recalculate NOW(), TODAY(), RAND(), RANDBETWEEN(), RANDARRAY()

### 12.3 Multi-Threaded Recalculation [P1]
- ✅ Distribute independent formula chains across CPU cores (QtConcurrent, level-parallel)
- ✅ Thread-safe built-in functions parallelized automatically (threshold: 64 cells)
...
- ✅ Detection and warning dialog (hasCircularDependency + detectCycle)
...
> **Current count: 365 registered functions**
...
- ✅ DATEVALUE, TIMEVALUE
- ✅ **DATEDIF** (interval: "Y","M","D","MD","YM","YD")
...
- ✅ FREQUENCY [P2]
- ✅ **Distribution functions** [P2]: NORM.DIST, NORM.INV, NORM.S.DIST, NORM.S.INV, T.DIST, T.DIST.2T, T.INV, T.INV.2T, CHISQ.DIST, CHISQ.INV, F.DIST, F.INV, BINOM.DIST, BINOM.INV, ⬜ POISSON.DIST, EXPON.DIST, GAMMA.DIST, GAMMA.INV, BETA.DIST, BETA.INV, LOGNORM.DIST, LOGNORM.INV, WEIBULL.DIST, NEGBINOM.DIST, HYPGEOM.DIST, CONFIDENCE.NORM, CONFIDENCE.T
...
- ✅ ERF, ERFC; ⬜ ERF.PRECISE, ERFC.PRECISE [P3]
...
- ✅ Spill range outlined with blue border (dotted blue border for spill children)
...
- ⬜ **Name Manager**: create, edit, delete, filter names (full UI)
...
- ✅ Combo chart (column + line on dual axes, per-series secondary Y-axis)
...
- ✅ Chart title (position, linked to cell, font/color with bold/italic formatting)
- ✅ Axis titles (horizontal + vertical)
- ✅ **Axis formatting**: min/max bounds, major/minor units, log scale, display units (thousands/millions/billions), reverse order, number format
- ✅ Legend (position: top/bottom/left/right/none, font, show/hide entries)
- ✅ Data labels (value/category/series/percentage, position, number format, font)
...
- ✅ **Data bars**: solid, positive/negative colors, bar direction, min/max type (auto), show/hide value
- ✅ **Color scales**: 2-color, 3-color; custom colors for min/midpoint/max
- ✅ **Icon sets**: 5 sets (3-traffic-lights, 3-arrows, 3-flags, 3-stars, 3-checkmarks); thresholds
- ✅ **Formula-based rules**: any formula returning TRUE/FALSE; can reference other cells
...
- ✅ Aggregation: SUM, COUNT, AVERAGE, MAX, MIN; ✅ Count Distinct; ⬜ PRODUCT, STDEV, VAR
- ⬜ Distinct Count (if using data model)
- ⬜ **Show Values As** (12+ options)
- ⬜ **Grouping**: dates, numbers, custom text groups
...
- ✅ Filter: value filters, manual item check/uncheck
...
- ✅ Number filters: equals, doesn't equal, greater than, between, top N, custom
...
- ✅ Table styles gallery (12 built-in themes)
...
### 19.1 Goal Seek [P1]
- ✅ Set cell to value by changing one input cell (dialog exists)
- ⬜ Iterative solving (execution not confirmed)
...
- ✅ JavaScript scripting (QJSEngine with SpreadsheetAPI)
...
- ✅ Read/write cell values, formulas, formatting from script (getCellValue, setCellValue, setBold, etc.)
...
- ⬜ REST API connector
- ✅ CSV/JSON/XML file import (CSV implemented; JSON/XML not yet)
...
- ✅ Natural language to formula conversion (suggestFormulas via ClaudeService)
- ⬜ Explain any formula (step-by-step breakdown)
- ⬜ Error detection and fix suggestions

### 22.4 Data AI [P1]
- ✅ Data analysis suggestions (analyzeData via ClaudeService)
- ⬜ Chart/PivotTable recommendations from selected data
- ⬜ Anomaly/outlier detection
- ⬜ Trend analysis and insights

### 22.5 AI Chat Panel [P1]
- ✅ Side panel for natural language interaction (ChatPanel)
...
### 23.2 Comments [P1]
- ✅ Comments on any cell (single text per cell)
- ⬜ Threaded comments (reply in thread)
...
- ✅ Auto-save with recovery (AutoSaveService + IncrementalSaveService)
...
- 🟡 Cell locking (lock/unlock per cell — UI exists, not enforced)
...
- ✅ **Formulas**: functions preserved; cell references; named ranges; array formulas; dynamic arrays
- ✅ **Formatting**: number formats (including custom codes); fonts; colors; borders (per-edge); fills; alignment; rotation; merged cells
- ✅ **Conditional formatting**: rules via Nexel JSON metadata; data bars; color scales; icon sets
- ✅ **Charts**: OOXML chart XML + Nexel JSON; all implemented chart types round-trip
- ⬜ **PivotTables**: field layout; aggregations; grouping; calculated fields; styles; cache
- ✅ **Data validation**: all types via Nexel JSON metadata; dropdown lists; input/error messages
- ⬜ **Images & shapes**: embedded images; position anchoring (widgets exist but no XLSX export)
- ✅ **Named ranges**: workbook scope (import confirmed)
...
- ⬜ **Comments**: not exported to XLSX (stored in Nexel metadata only)

### 24.2 Other Formats [P1]
- ✅ CSV/TSV import with intelligent delimiter + encoding detection (progressive loading, mmap)
- ✅ CSV/TSV export
...
### 26.1 Protection [P1]
- ⬜ Sheet protection with configurable allowed actions (UI exists, NOT enforced)
- ⬜ Workbook structure protection
- 🟡 Cell lock/unlock (metadata stored, not enforced)
...
| Layer | P1 | P2 | P3 | Total | Done |
|-------|----|----|-----|-------|------|
| Grid & Cell UX | ~60 | ~18 | ~7 | ~85 | ~45 |
| Compute Engine | ~15 | ~5 | ~2 | ~22 | ~15 |
| Formula Functions | ~350 | ~80 | ~45 | ~475 | ~365 |
| Dynamic Arrays/Spill | ~8 | ~2 | — | ~10 | ~5 |
| References & Names | ~15 | ~8 | ~2 | ~25 | ~8 |
| Charts & Cond Format | ~65 | ~30 | ~8 | ~103 | ~55 |
| PivotTables & Analysis | ~50 | ~25 | ~5 | ~80 | ~8 |
| Automation & Scripting | ~20 | ~10 | ~5 | ~35 | ~8 |
| Data Engine (ETL) | ~25 | ~15 | ~5 | ~45 | ~1 |
| AI & Copilot | ~18 | ~5 | ~2 | ~25 | ~4 |
| Collaboration | ~25 | ~8 | ~2 | ~35 | ~3 |
| File Formats | ~20 | ~10 | — | ~30 | ~12 |
| Print & Page Layout | ~2 | ~15 | — | ~17 | 0 |
| Security & Enterprise | ~5 | ~8 | ~2 | ~15 | ~1 |
| **TOTAL** | **~678** | **~239** | **~85** | **~1002** | **~530** |

> **Audit date**: 2026-03-30
> **Estimated completion**: ~530 of 1002 features (~53%)
> **P1 completion**: ~370 of 678 P1 features (~55%)
> **Key strengths**: Formula engine (365 functions), number formats, charts, conditional formatting, data validation, virtual scrolling
> **Critical gaps**: Printing/PDF, sheet protection enforcement, multi-level sort, row/column grouping, structured table references, collaboration, Power Query equivalent
